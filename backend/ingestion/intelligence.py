...
def extract_intelligence(parsed: ParsedDocument) -> IntelligenceExtractionResult:
    ...
    projects = _unique(
        re.findall(r"\b(?:Project|Program|Initiative)\s+[A-Z][A-Za-z0-9_-]+\b", text)
    )[:30]
    locations = _unique(
        re.findall(r"\b(?:in|at)\s+([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+){0,2})\b", text)
    )[:20]

    obligations = _matching(
        sentences, ("shall", "must", "required to", "responsible for", "obligated")
    )
    risks = _matching(
        sentences,
        ("risk", "penalty", "breach", "non-compliance", "liability", "failure"),
    )
    action_items = _matching(
        sentences, ("action item", "todo", "follow up", "owner:", "next step")
    )
    decisions = _matching(
        sentences, ("decided", "decision", "approved", "agreed", "resolved")
    )
    policy_requirements = _matching(
        sentences,
        ("policy requires", "must comply", "control", "standard", "procedure"),
    )
    missing = _matching(
        sentences, ("tbd", "missing", "unknown", "to be provided", "not available")
    )
    deadline_sentences = _matching(
        sentences, ("due", "deadline", "by ", "no later than", "expires", "renewal")
    )
    ...
    return IntelligenceExtractionResult(
        ...
        deadlines=[
            {"date": date, "evidence": _first_sentence_with(sentences, date)}
