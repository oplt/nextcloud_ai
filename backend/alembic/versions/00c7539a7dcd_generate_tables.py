...
def upgrade() -> None:
    """Upgrade schema."""
    # Idempotent: required before VECTOR columns/indexes on a fresh database.
    # Existing deployments that already applied this revision are covered by
    # the forward migration `a1b2c3d4e5f6_ensure_vector_extension`.
    ...
    op.create_table(
        ...
        sa.Column(
            "memory_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("id", sa.UUID(), nullable=False),
        sa.Column(
            "created_at",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["owner_user_id"],
            ["users.id"],
            name=op.f("fk_connectors_owner_user_id_users"),
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_connectors")),
    )
    op.create_index(
        op.f("ix_connectors_owner_user_id"),
        "connectors",
        ["owner_user_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "intelligence_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_documents_business_domain", "documents", ["business_domain"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "ix_documents_document_type", "documents", ["document_type"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        op.f("ix_knowledge_nodes_label"), "knowledge_nodes", ["label"], unique=False
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_knowledge_edges_target_node_id"), table_name="knowledge_edges"
    )
    op.drop_index(
        op.f("ix_knowledge_edges_source_node_id"), table_name="knowledge_edges"
    )
    op.drop_index(
        op.f("ix_knowledge_edges_relation_type"), table_name="knowledge_edges"
    )
    ...
    op.drop_index(
        op.f("ix_document_insights_insight_type"), table_name="document_insights"
    )
    op.drop_index("ix_document_insights_document_type", table_name="document_insights")
    op.drop_index(
        op.f("ix_document_insights_document_id"), table_name="document_insights"
    )
    op.drop_table("document_insights")
    op.drop_index(
        op.f("ix_document_chunks_embedding_status"), table_name="document_chunks"
    )
    op.drop_index(
        "ix_document_chunks_embedding_ann",
