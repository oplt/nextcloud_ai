...
@pytest.mark.asyncio
async def test_enqueue_connector_sync_job_falls_back_to_local_execution_without_worker(
    monkeypatch,
) -> None:
    event = asyncio.Event()
    calls: list[tuple[str, str | None]] = []
    ...
    monkeypatch.setattr(indexing_tasks.settings, "APP_ENV", "development")
    monkeypatch.setattr(indexing_tasks, "_celery_worker_is_available", lambda: False)

    handle = indexing_tasks.enqueue_connector_sync_job("job-456")

    await asyncio.wait_for(event.wait(), timeout=1)

    assert handle.id
    assert calls == [("job-456", handle.id)]


@pytest.mark.asyncio
async def test_enqueue_document_reindex_falls_back_to_local_execution_without_worker(
    monkeypatch,
) -> None:
    event = asyncio.Event()
    calls: list[str] = []
    ...
    handle = indexing_tasks.enqueue_document_reindex("doc-456")

    await asyncio.wait_for(event.wait(), timeout=1)

    assert handle.id
    assert calls == ["doc-456"]
